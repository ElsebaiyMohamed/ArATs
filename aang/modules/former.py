    ...
    def forward(self, x, padding_mask=None, need_weights=False, attn_mask=None, training=False):
        if need_weights:
            ...
            for layer in self.enc_stack:
                x, atten_weights = layer(x, padding_mask=padding_mask, need_weights=need_weights, attn_mask=attn_mask, training=training)
            return x, atten_weights
        
        for layer in self.enc_stack:
                x = layer(x, padding_mask=padding_mask, need_weights=need_weights, attn_mask=attn_mask, training=training)
        return x
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, query, key, query_mask=None, key_mask=None, need_weights=False, attn_mask=None, training=False):
        if query.dim() < 3:
        ...
        att_out, _ = self.smha(query, query, query, key_padding_mask=query_mask, need_weights=False, attn_mask=attn_mask)
        ...
        att_out, att_weight = self.cmha(query, key, key, key_padding_mask=key_mask, need_weights=need_weights, attn_mask=None)
        ...
        if need_weights:
            for layer in self.dec_stack:
                query, atten_weight = layer(query, key, query_mask=query_mask, key_mask=key_mask, need_weights=need_weights,
                                            attn_mask=attn_mask, training=training)
            return query, atten_weight
        
        for layer in self.dec_stack:
            query = layer(query, key, query_mask=query_mask, key_mask=key_mask, need_weights=need_weights, 
                          attn_mask=attn_mask, training=training)
