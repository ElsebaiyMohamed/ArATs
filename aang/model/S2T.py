...
class Model(nn.Module):  
    ...
    def forward(self, wave, target: dict, training=False, wave_mask=False, need_enc_weights=False, enc_mask=False,
        ...
        enc_mask = self.look_ahead_mask(wave.size(1), wave.size(1), wave.device) if enc_mask  else None
        wave = self.context_enc(wave, padding_mask=wave_mask, need_weights=need_enc_weights, attn_mask=enc_mask, training=training)
        ...
        for h, _ in self.hparams.head_names.items():
            ...
            att_mask = self.look_ahead_mask(target_head.size(1), target_head.size(1), target_head.device) if dec_mask  else None
            
            
            target_head = self.context_dec(query=target_head, key=wave, query_mask=query_mask, key_mask=wave_mask, 
                                              need_weights=need_dec_weights, attn_mask=att_mask, training=training)
...
#         self.model = torch.compile(self.model, dynamic=True)
    ...
    def training_step(self, batch, batch_idx):
        ...
        for h, pad_idx in self.hparams.head_names.items():
            ...
            self.log(f'{h}_Loss', h_loss, on_step=True, prog_bar=True, on_epoch=True, logger=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for h, pad_idx in self.hparams.head_names.items():
            h_loss = F.cross_entropy(results[h]['predection'].transpose(2, 1), ground_truth[h], reduction='sum')
            loss += h_loss
            self.log(f'{h}_{at}_Loss', h_loss, on_step=False, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
        
        self.log(f"{at}_Loss", loss, on_step=False, on_epoch=True, prog_bar=True, logger=True, sync_dist=True)
